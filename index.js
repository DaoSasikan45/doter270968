...
// ฟังก์ชันสร้างตาราง MedicationReminders แบบบังคับ
const forceCreateMedicationReminderTable = async () => {
  try {
    console.log('🔄 Force creating MedicationReminders table...');
    
    // 1. ลบตารางเก่าออกก่อน (ถ้ามี)
    await pool.execute('SET FOREIGN_KEY_CHECKS = 0');
    await pool.execute('DROP TABLE IF EXISTS MedicationReminders');
    await pool.execute('SET FOREIGN_KEY_CHECKS = 1');
    console.log('🗑️ Dropped existing MedicationReminders table');
    ...
    await pool.execute(`
      CREATE TABLE MedicationReminders (
        ...
        taken_status ENUM('pending', 'taken', 'skipped', 'overdue') DEFAULT 'pending',
        ...
        INDEX idx_reminder_date (scheduled_date),
        INDEX idx_reminder_status (taken_status)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    `);
    
    console.log('✅ MedicationReminders table created successfully with all columns!');
    
    // 3. ตรวจสอบโครงสร้างตาราง
    const [columns] = await pool.execute('DESCRIBE MedicationReminders');
    console.log('📋 Table structure:', columns.map(col => col.Field));
    
    return true;
    
  } catch (error) {
    console.error('❌ Error force creating MedicationReminders table:', error);
    console.error('❌ Full error details:', error.message);
    return false;
  }
};

// Test database connection และสร้างตาราง
pool.getConnection()
  .then(async (connection) => {
    ...
    // สร้างตารางทันที
    const tableCreated = await forceCreateMedicationReminderTable();
    if (tableCreated) {
      console.log('✅ Ready to handle medication API requests');
    } else {
      console.log('❌ Medication APIs may not work properly');
    }
  })
  .catch(err => {
...
// เพิ่มฟังก์ชันนี้ในไฟล์ index.js หลังจาก pool.getConnection() test

const fixDatabaseSchema = async () => {
  try {
    console.log('🔧 Fixing database schema...');
    
    // แก้ไขขนาดคอลัมน์ relationship_id
    await pool.execute(`
      ALTER TABLE DoctorPatientRelationships 
      MODIFY COLUMN relationship_id VARCHAR(36) NOT NULL
    `);
    console.log('✅ Fixed DoctorPatientRelationships.relationship_id to VARCHAR(36)');
    
    // แก้ไขคอลัมน์อื่นๆ ที่อาจมีปัญหาเดียวกัน
    const fixes = [
      'ALTER TABLE DoctorPatientRelationships MODIFY COLUMN doctor_id VARCHAR(36) NOT NULL',
      'ALTER TABLE DoctorPatientRelationships MODIFY COLUMN patient_id VARCHAR(36) NOT NULL',
      'ALTER TABLE PatientMedications MODIFY COLUMN prescription_id VARCHAR(36) NOT NULL',
      'ALTER TABLE PatientMedications MODIFY COLUMN patient_id VARCHAR(36) NOT NULL',
      'ALTER TABLE PatientMedications MODIFY COLUMN medication_id VARCHAR(36) NOT NULL',
      'ALTER TABLE PatientMedications MODIFY COLUMN doctor_id VARCHAR(36) NOT NULL',
      'ALTER TABLE Medications MODIFY COLUMN medication_id VARCHAR(36) NOT NULL'
    ];
    
    for (const sql of fixes) {
      try {
        await pool.execute(sql);
        console.log(`✅ Fixed: ${sql.substring(0, 60)}...`);
      } catch (error) {
        // ข้ามถ้าคอลัมน์ถูกต้องอยู่แล้ว
...
// เรียกใช้ฟังก์ชันนี้หลังจาก database connection test
pool.getConnection()
  .then(async (connection) => {
    console.log('✅ Database connection successful');
    connection.release();
    ...
    await forceCreateMedicationReminderTable(); // ถ้ามีฟังก์ชันนี้อยู่แล้ว
    await createIOPMeasurementsTable(); // ถ้ามีฟังก์ชันนี้อยู่แล้ว
    
    console.log('🚀 System ready to handle medication prescriptions');
  })
  .catch(err => {
    console.error('❌ Database connection failed:', err.message);
  });
...
const createIOPMeasurementsTable = async () => {
  try {
    ...
    `);
    console.log('✅ IOP_Measurements table checked/created.');
  } catch (error) {
    console.log('⚠️ IOP_Measurements table issue:', error.message);
  }
};

// 2. ฟังก์ชันแก้คอลัมน์ measurement_id (มีแค่ 1 อัน)
const fixMeasurementIdColumn = async () => {
  try {
    console.log('🔧 Attempting to fix IOP_Measurements.measurement_id column size...');
    await pool.execute(`
      ALTER TABLE IOP_Measurements MODIFY COLUMN measurement_id VARCHAR(36)
    `);
    console.log('✅ IOP_Measurements.measurement_id column fixed to VARCHAR(36).');
  } catch (error) {
    // ไม่ต้องแสดง error เพราะถ้าคอลัมน์ถูกต้องอยู่แล้วก็จะเข้า catch นี้
    // console.log('⚠️ Could not alter IOP_Measurements.measurement_id column (might be correct already).');
  }
};

// 3. ฟังก์ชันแก้คอลัมน์ prescription_id (มีแค่ 1 อัน)
const fixPrescriptionIdColumn = async () => {
  try {
    console.log('🔧 Attempting to fix PatientMedications.prescription_id column size...');
    await pool.execute(`
      ALTER TABLE PatientMedications MODIFY COLUMN prescription_id VARCHAR(36)
    `);
    console.log('✅ PatientMedications.prescription_id column fixed to VARCHAR(36).');
  } catch (error) {
    // ไม่ต้องแสดง error เพราะถ้าคอลัมน์ถูกต้องอยู่แล้วก็จะเข้า catch นี้
    // console.log('⚠️ Could not alter PatientMedications.prescription_id column (might be correct already).');
  }
};


// ==============================================================
// === กลุ่มที่เรียกใช้ฟังก์ชันตอนเริ่มเซิร์ฟเวอร์ (วางรวมกันตรงนี้) ===
// ==============================================================

// [สำคัญ] ให้มี setTimeout เรียกแต่ละฟังก์ชันแค่ครั้งเดียวตามลำดับนี้
setTimeout(() => {
  createIOPMeasurementsTable();
}, 1000);

setTimeout(() => {
  fixMeasurementIdColumn();
}, 1500); 

setTimeout(() => {
  fixPrescriptionIdColumn();
}, 1600);
...
const fixTreatmentPlansTable = async () => {
  try {
    console.log('🔧 Checking GlaucomaTreatmentPlans column structure...');
    
    // ตรวจสอบโครงสร้างตารางปัจจุบัน
    const [columns] = await pool.execute('DESCRIBE GlaucomaTreatmentPlans');
    const treatmentPlanIdColumn = columns.find(col => col.Field === 'treatment_plan_id');
    
    console.log('📋 Current treatment_plan_id column:', treatmentPlanIdColumn);
    
    if (treatmentPlanIdColumn && treatmentPlanIdColumn.Type !== 'varchar(36)') {
      console.log('⚠️  Column size is wrong, attempting to fix...');
      
      // ลองแก้ไขขนาด column ก่อน
      await pool.execute(`
        ALTER TABLE GlaucomaTreatmentPlans 
        MODIFY COLUMN treatment_plan_id VARCHAR(36) NOT NULL
      `);
      console.log('✅ treatment_plan_id column fixed to VARCHAR(36)');
    }
    
    // แก้ไข treatment_approach ด้วย
    await pool.execute(`
      ALTER TABLE GlaucomaTreatmentPlans 
    ...
    try {
      console.log('🗑️  Dropping and recreating GlaucomaTreatmentPlans table...');
...
const fixMedicationIdColumns = async () => {
  try {
    console.log('🔧 Attempting to fix medication_id column sizes...');
    // แก้ไขตารางหลัก
    await pool.execute(`ALTER TABLE Medications MODIFY COLUMN medication_id VARCHAR(36)`);
    console.log('✅ Medications.medication_id column fixed.');
    // แก้ไขตารางที่อ้างอิง
    await pool.execute(`ALTER TABLE PatientMedications MODIFY COLUMN medication_id VARCHAR(36)`);
    console.log('✅ PatientMedications.medication_id column fixed.');
  } catch (error) {
    // console.log('⚠️ Could not alter medication_id columns (might be correct already).');
  }
};

// เพิ่มฟังก์ชันใหม่นี้เข้าไป
const fixAppointmentIdColumns = async () => {
  try {
    console.log('🔧 Attempting to fix Appointment ID column sizes...');
    await pool.execute(`ALTER TABLE Appointments MODIFY COLUMN appointment_id VARCHAR(36) NOT NULL`);
    await pool.execute(`ALTER TABLE Appointments MODIFY COLUMN patient_id VARCHAR(36) NOT NULL`);
    await pool.execute(`ALTER TABLE Appointments MODIFY COLUMN doctor_id VARCHAR(36) NOT NULL`);
    await pool.execute(`ALTER TABLE Appointments MODIFY COLUMN created_by VARCHAR(36) NOT NULL`);
    console.log('✅ Appointments table ID columns fixed to VARCHAR(36).');
  } catch (error) {
    console.log('⚠️ Could not alter Appointments ID columns (might be correct already):', error.message);
  }
};


// ปรับปรุง endpoint สำหรับสร้าง treatment plan
...
app.post('/api/patients/:patientId/treatment-plans', authDoctor, async (req, res) => {
  ...
  try {
    ...
    // ทำให้แผนการรักษาที่ยังใช้งานอยู่เป็น completed
    ...
    // ตรวจสอบความยาวของ UUID
    console.log('🔍 Generated UUID length:', treatmentPlanId.length);
    
    if (treatmentPlanId.length > 36) {
      throw new Error('UUID too long for database column');
    }
    ...
    if (error.code === 'ER_DATA_TOO_LONG') {
      errorMessage = 'Database column size issue - please contact administrator';
...
const createSurgeryTable = async () => {
  try {
    await pool.execute(`
      CREATE TABLE IF NOT EXISTS GlaucomaSurgeries (
        ...
        pre_op_iop_left DECIMAL(4,1) NULL,
        pre_op_iop_right DECIMAL(4,1) NULL,
        ...
        INDEX idx_surgery_doctor (doctor_id),
        INDEX idx_surgery_date (surgery_date)
      )
    `);
    console.log('✅ GlaucomaSurgeries table checked/created.');
  } catch (error) {
    console.log('⚠️ GlaucomaSurgeries table issue:', error.message);
  }
};

// สร้างตาราง GlaucomaTreatmentPlans
const createTreatmentPlansTable = async () => {
  try {
    await pool.execute(`
      CREATE TABLE IF NOT EXISTS GlaucomaTreatmentPlans (
        treatment_plan_id VARCHAR(36) PRIMARY KEY,
        patient_id VARCHAR(36) NOT NULL,
        doctor_id VARCHAR(36) NOT NULL,
        start_date DATE NOT NULL,
    ...
    `);
    console.log('✅ GlaucomaTreatmentPlans table checked/created.');
  } catch (error) {
    console.log('⚠️ GlaucomaTreatmentPlans table issue:', error.message);
  }
};
...
pool.getConnection()
  .then(async (connection) => {
    ...
    // สร้างตารางที่จำเป็น
    await createSurgeryTable();
    await createTreatmentPlansTable();
    
    console.log('🚀 Surgery and Treatment Plan APIs ready');
  })
  .catch(err => {
    console.error('❌ Database connection failed:', err.message);
  });

// แก้ไข Surgery API endpoint
app.post('/api/patients/:patientId/surgeries', authDoctor, async (req, res) => {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();
    console.log('🔄 Surgery transaction started');
    ...
    console.log('🏥 Creating surgery record:', {
      patientId, doctorId, surgeryDate, surgeryType, eye
    });
    ...
    const surgeryId = require('crypto').randomUUID();

    await connection.execute(
      `INSERT INTO GlaucomaSurgeries (
        surgery_id, patient_id, doctor_id, surgery_date, surgery_type, eye,
        procedure_details, complications, outcome, notes
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        surgeryId, patientId, doctorId, surgeryDate, surgeryType, eye || null,
        procedureDetails || null, complications || null, outcome || 'pending', notes || null
      ]
    );

    await connection.commit();
    console.log(`✅ Surgery record created: ${surgeryId}`);
    ...
    res.status(500).json({ 
      error: 'Failed to create surgery record: ' + error.message 
    });
  } finally {
    connection.release();
  }
...
// แก้ไข Treatment Plan API endpoint  
app.post('/api/patients/:patientId/treatment-plans', authDoctor, async (req, res) => {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();
    console.log('🔄 Treatment plan transaction started');
    ...
    if (!treatmentApproach) {
      await connection.rollback();
      return res.status(400).json({ error: 'Treatment approach is required' });
    }
    ...
    // สร้างหรือค้นหาความสัมพันธ์ doctor-patient
    let [relationship] = await connection.execute(
      `SELECT relationship_id FROM DoctorPatientRelationships
       WHERE doctor_id = ? AND patient_id = ?`,
      [doctorId, patientId]
    ...
    // สร้างแผนการรักษาใหม่
    const treatmentPlanId = require('crypto').randomUUID();

    await connection.execute(
      `INSERT INTO GlaucomaTreatmentPlans (
        treatment_plan_id, patient_id, doctor_id, start_date,
        treatment_approach, target_iop_left, target_iop_right,
        follow_up_frequency, visual_field_test_frequency, notes, status
      ) VALUES (?, ?, ?, CURDATE(), ?, ?, ?, ?, ?, ?, 'active')`,
      [
        treatmentPlanId, patientId, doctorId,
        treatmentApproach,
        targetIOPLeft ? parseFloat(targetIOPLeft) : null,
        targetIOPRight ? parseFloat(targetIOPRight) : null,
        followUpFrequency || null,
        visualFieldTestFrequency || null,
        notes || null
      ]
    ...
    await connection.commit();
    console.log(`✅ Treatment plan created: ${treatmentPlanId}`);
    ...
    console.error('❌ Error creating treatment plan:', error);
    res.status(500).json({ 
      error: 'Failed to create treatment plan: ' + error.message 
    });
  } finally {
    connection.release();
  }
});
// เพิ่มในส่วน database initialization ของ index.js (หลังจาก pool.getConnection())
// Test database connection และสร้างตารางที่จำเป็น
pool.getConnection()
  .then(async (connection) => {
    console.log('✅ Database connection successful');
    connection.release();
    
    // 🔧 แก้ไขโครงสร้างตารางทั้งหมด
    console.log('🔧 Starting database structure fixes...');
    
    // 1. แก้ไข GlaucomaTreatmentPlans table (สำคัญที่สุด!)
    await fixTreatmentPlansTable();
    
    // 2. แก้ไข medication columns
    await fixMedicationIdColumns();
    
    // 3. แก้ไข appointment columns  
    await fixAppointmentIdColumns();
    
    // 4. แก้ไข IOP table
    await fixIOPTable();
    
    // 5. สร้างตาราง MedicationReminders
    const tableCreated = await forceCreateMedicationReminderTable();
    
    if (tableCreated) {
      console.log('✅ All database structures fixed and ready!');
      console.log('🎯 System ready to handle all API requests');
    } else {
      console.log('⚠️  Some APIs may not work properly');
    }
...
app.get('/api/patients/:patientId/medications/:prescriptionId', authDoctor, async (req, res) => {
  try {
    const { patientId, prescriptionId } = req.params;
    const doctorId = req.doctor.doctor_id;
    
    console.log(`📋 Getting medication details: ${prescriptionId} for patient: ${patientId}`);
    
    // ตรวจสอบว่าผู้ป่วยอยู่ในความดูแลของหมอหรือไม่
    const [patientCheck] = await pool.execute(
      `SELECT patient_id FROM DoctorPatientRelationships 
       WHERE doctor_id = ? AND patient_id = ? AND status = 'active'`,
      [doctorId, patientId]
    );
    
    if (patientCheck.length === 0) {
      console.log(`❌ Access denied - patient not under doctor's care`);
      return res.status(403).json({ error: 'Access denied to this patient' });
    }
    
    console.log(`✅ Doctor-patient relationship verified`);
    
    // ดึงข้อมูลยาจากตาราง PatientMedications (ใช้ column names ที่ถูกต้อง)
    ...
    const medicationData = medication[0];
    console.log(`✅ Found medication with ID: ${medicationData.medication_id}`);
    
    // ดึงชื่อยาจากตาราง Medications (ถ้ามี)
    let medicationName = null;
    let genericName = null;
    
    try {
      const [medicationInfo] = await pool.execute(
      ...
      if (medicationInfo.length > 0) {
        medicationName = medicationInfo[0].medication_name;
        genericName = medicationInfo[0].generic_name;
        console.log(`✅ Found medication name: ${medicationName}`);
      }
    } catch (medicationError) {
      console.log('⚠️ Could not fetch medication name from Medications table:', medicationError.message);
    }
    
    // จัดรูปแบบข้อมูลให้ส่งกลับ
    const result = {
      ...
      medication_name: medicationName || `Medication-${medicationData.medication_id}`,
      generic_name: genericName,
      category: null,
      form: null,
      strength: medicationData.concentration,
      concentration: medicationData.concentration,
      dosage: medicationData.dosage,
      frequency: medicationData.frequency,
      eye: medicationData.eye || medicationData.eye_selection,
      ...
      prescribed_by: medicationData.doctor_id,
      status: medicationData.status,
      duration: medicationData.duration,
      frequency_type: medicationData.frequency_type,
      frequency_value: medicationData.frequency_value,
      instruction_notes: medicationData.instruction_notes,
    ...
    console.log(`✅ Medication details prepared successfully`);
    res.json(result);
...
// PUT edit medication details (Safe Version)
app.put('/api/patients/:patientId/medications/:prescriptionId/edit', authDoctor, async (req, res) => {
  try {
    const { patientId, prescriptionId } = req.params;
    const { 
      eye, 
    ...
    console.log(`📝 Editing medication: ${prescriptionId} for patient: ${patientId}`);
    ...
    // ตรวจสอบว่ายาเป็นของผู้ป่วยหรือไม่
    const [medicationCheck] = await pool.execute(
      `SELECT prescription_id FROM PatientMedications 
       WHERE prescription_id = ? AND patient_id = ?`,
      [prescriptionId, patientId]
    );
    ...
    if (!eye || !dosage || !frequency) {
      return res.status(400).json({ error: 'Eye, dosage, and frequency are required' });
    }
    
    // อัพเดทข้อมูลยา (columns หลักที่มีแน่นอน)
    const [result] = await pool.execute(
      `UPDATE PatientMedications 
       SET eye = ?, dosage = ?, frequency = ?, start_date = ?, end_date = ?, 
           special_instructions = ?, updated_at = CURRENT_TIMESTAMP
       WHERE prescription_id = ? AND patient_id = ?`,
      [eye, dosage, frequency, start_date, end_date, special_instructions, prescriptionId, patientId]
    );
    
    // อัพเดทความเข้มข้น (ถ้ามี column concentration)
    if (concentration) {
      try {
        await pool.execute(
          `UPDATE PatientMedications 
           SET concentration = ? 
           WHERE prescription_id = ? AND patient_id = ?`,
          [concentration, prescriptionId, patientId]
        );
        console.log(`✅ Updated concentration: ${concentration}`);
      } catch (concentrationError) {
        console.log('Note: concentration field may not exist, skipping...', concentrationError.message);
      }
    }
    
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'No changes made to medication' });
    }
    
    console.log(`✅ Medication edited successfully: ${prescriptionId}`);
    console.log(`   - Eye: ${eye}`);
    ...
    res.status(500).json({ 
      error: 'Failed to edit medication',
      details: error.message 
    });
  }
});

console.log('✅ Safe medication edit endpoints added successfully (without Drugs table dependency)');
console.log('📋 Available endpoints:');
console.log('   GET /api/patients/:patientId/medications/:prescriptionId - Get medication details');
console.log('   PUT /api/patients/:patientId/medications/:prescriptionId/edit - Edit medication details');


// PUT edit medication details
app.put('/api/patients/:patientId/medications/:prescriptionId/edit', authDoctor, async (req, res) => {
  try {
    const { patientId, prescriptionId } = req.params;
    const { 
    ...
    // อัพเดทความเข้มข้น (ถ้ามี column concentration ในตาราง)
    if (concentration) {
      try {
        await pool.execute(
          `UPDATE PatientMedications 
...
// DELETE/Discontinue medication (ถ้าต้องการเพิ่มฟีเจอร์หยุดยา)
app.put('/api/patients/:patientId/medications/:prescriptionId/discontinue', authDoctor, async (req, res) => {
  try {
    const { patientId, prescriptionId } = req.params;
    const { reason } = req.body;
    const doctorId = req.doctor.doctor_id;
    
    console.log(`🛑 Discontinuing medication: ${prescriptionId} for patient: ${patientId}`);
    ...
    // อัพเดทสถานะยาเป็น discontinued
    const [result] = await pool.execute(
      `UPDATE PatientMedications 
       SET status = 'discontinued', 
           end_date = CURDATE(),
           special_instructions = CONCAT(COALESCE(special_instructions, ''), 
                                       IF(special_instructions IS NOT NULL AND special_instructions != '', '\n', ''),
                                       'หยุดใช้: ', COALESCE(?, 'ไม่ระบุเหตุผล')),
           updated_at = CURRENT_TIMESTAMP
       WHERE prescription_id = ? AND patient_id = ? AND status = 'active'`,
      [reason, prescriptionId, patientId]
    );
    
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Medication not found or already discontinued' });
    }
    
    console.log(`✅ Medication discontinued successfully: ${prescriptionId}`);
...
console.log('✅ Medication edit endpoints added successfully');
console.log('📋 Available medication edit endpoints:');
console.log('   GET /api/patients/:patientId/medications/:prescriptionId - Get single medication details');
console.log('   PUT /api/patients/:patientId/medications/:prescriptionId/edit - Edit medication details');
console.log('   PUT /api/patients/:patientId/medications/:prescriptionId/discontinue - Discontinue medication');

// DELETE API สำหรับลบข้อมูลยาผู้ป่วยออกจากฐานข้อมูลอย่างสมบูรณ์
app.delete('/api/patients/:patientId/medications/:prescriptionId', authDoctor, async (req, res) => {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();
    ...
    console.log(`🗑️ Attempting to delete medication: ${prescriptionId} for patient: ${patientId}`);
    ...
    // ตรวจสอบว่ายาที่จะลบมีอยู่จริงและเป็นของหมอคนนี้
    const [medicationCheck] = await connection.execute(
      `SELECT pm.prescription_id, pm.patient_id, pm.status, m.name as medication_name
       FROM PatientMedications pm
       LEFT JOIN Medications m ON pm.medication_id = m.medication_id
       WHERE pm.prescription_id = ? AND pm.patient_id = ? AND pm.doctor_id = ?`,
    ...
    const medicationData = medicationCheck[0];
    console.log(`📋 Found medication to delete: ${medicationData.medication_name} (Status: ${medicationData.status})`);
    ...
    if (deleteResult.affectedRows === 0) {
      await connection.rollback();
      return res.status(404).json({ error: 'Failed to delete medication - no records affected' });
    }
    
    await connection.commit();
    console.log(`✅ Medication deleted successfully: ${prescriptionId}`);
    ...
    if (error.code === 'ER_ROW_IS_REFERENCED_2') {
      errorMessage = 'Cannot delete medication - it is referenced by other records';
    } else if (error.code === 'ER_NO_REFERENCED_ROW_2') {
      errorMessage = 'Invalid reference data';
    } else if (error.sqlMessage) {
      errorMessage = 'Database error: ' + error.sqlMessage;
...
console.log('✅ Delete medication API endpoint added successfully');
console.log('🔧 Available medication delete endpoint:');
console.log('   DELETE /api/patients/:patientId/medications/:prescriptionId - Delete medication permanently');
console.log('⚠️  WARNING: This performs a hard delete - data cannot be recovered');
...
// ===========================================
// MEDICATION REMINDER & ALERTS API
// เพิ่มส่วนนี้ในไฟล์ index.js ของคุณ หลังจาก line ที่มี Dashboard APIs
// ===========================================


// API: ดึงข้อมูลการแจ้งเตือนยาสำหรับ Dashboard
app.get('/api/medication/alerts', authDoctor, async (req, res) => {
  try {
    const { date, status, limit } = req.query;
    const currentDate = date || new Date().toISOString().split('T')[0];

    console.log(`💊 Loading medication alerts for date: ${currentDate}`);

    // ดึงข้อมูลยาที่ต้องหยอดวันนี้จาก PatientMedications ที่ active
    let baseQuery = `
      SELECT 
        pm.prescription_id,
        pm.patient_id,
        ...
        -- ตรวจสอบว่าหยอดยาแล้วหรือยัง
        CASE 
          WHEN mr.taken_status = 'taken' THEN 'taken'
          WHEN mr.taken_status = 'skipped' THEN 'skipped'
          WHEN mr.taken_status = 'overdue' THEN 'overdue'
          WHEN mr.reminder_id IS NULL THEN 'pending'
          ELSE mr.taken_status
        END as status,
        mr.taken_at,
        mr.notes as reminder_notes,
        mr.scheduled_time,
      ...
      JOIN Medications m ON pm.medication_id = m.medication_id
      LEFT JOIN DoctorProfiles d ON pm.doctor_id = d.doctor_id
      LEFT JOIN MedicationReminders mr ON pm.prescription_id = mr.prescription_id 
    ...
    let queryParams = [currentDate, currentDate, currentDate];

    // เพิ่มเงื่อนไขการกรองตาม status ถ้ามี
    if (status && status !== 'all') {
      if (status === 'pending') {
        baseQuery += ` AND (mr.taken_status IS NULL OR mr.taken_status = 'pending')`;
      } else {
        baseQuery += ` AND mr.taken_status = ?`;
    ...
    baseQuery += ` ORDER BY 
      CASE 
        WHEN mr.taken_status = 'overdue' THEN 1
        WHEN mr.taken_status IS NULL OR mr.taken_status = 'pending' THEN 2
        WHEN mr.taken_status = 'taken' THEN 3
        ELSE 4
      END,
    ...
    const [alerts] = await pool.execute(baseQuery, queryParams);

    // จัดกลุ่มข้อมูลตามสถานะ
    const groupedAlerts = {
      overdue: alerts.filter(a => a.status === 'overdue'),
      pending: alerts.filter(a => a.status === 'pending' || a.status === null),
      taken: alerts.filter(a => a.status === 'taken'),
      skipped: alerts.filter(a => a.status === 'skipped'),
      total: alerts.length
    };

    // สถิติสำหรับ Dashboard
    const summary = {
      total_patients: new Set(alerts.map(a => a.patient_id)).size,
      total_medications: alerts.length,
      overdue_count: groupedAlerts.overdue.length,
      pending_count: groupedAlerts.pending.length,
      taken_count: groupedAlerts.taken.length,
      completion_rate: alerts.length > 0 ? 
        ((groupedAlerts.taken.length / alerts.length) * 100).toFixed(1) : 0
    };

    console.log(`✅ Found ${alerts.length} medication alerts`);
...
// API: บันทึกการหยอดยาแล้ว
app.post('/api/medication/mark-taken', authDoctor, async (req, res) => {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();

    const { prescription_id, patient_id, notes } = req.body;
    const currentDate = new Date().toISOString().split('T')[0];
    const currentDateTime = new Date();

    console.log(`💊 Marking medication as taken:`, { prescription_id, patient_id });

    // ตรวจสอบว่ายามีอยู่จริง
    const [medicationExists] = await connection.execute(
      `SELECT pm.prescription_id, pm.patient_id, m.name as medication_name
       FROM PatientMedications pm
       JOIN Medications m ON pm.medication_id = m.medication_id
    ...
    // ตรวจสอบว่ามี reminder สำหรับวันนี้หรือยัง
    let [existingReminder] = await connection.execute(
      `SELECT reminder_id FROM MedicationReminders
       WHERE prescription_id = ? AND scheduled_date = ?`,
      [prescription_id, currentDate]
    );
    ...
    if (existingReminder.length === 0) {
      ...
      await connection.execute(
        `INSERT INTO MedicationReminders (
          ...
          scheduled_date, scheduled_time, taken_status, taken_at, notes
        ) VALUES (?, ?, ?, ?, ?, TIME(NOW()), 'taken', NOW(), ?)`,
        [
          reminderId, 
          prescription_id, 
          patient_id, 
          medicationExists[0].medication_name,
          currentDate, 
          notes || 'บันทึกผ่านระบบ'
        ]
      );
    } else {
      // อัปเดต reminder ที่มีอยู่
      reminderId = existingReminder[0].reminder_id;
      await connection.execute(
        `UPDATE MedicationReminders 
         SET taken_status = 'taken', taken_at = NOW(), notes = ?, updated_at = NOW()
         WHERE reminder_id = ?`,
        [notes || 'บันทึกผ่านระบบ', reminderId]
      );
    }

    await connection.commit();

    console.log(`✅ Medication marked as taken: ${medicationExists[0].medication_name}`);

    res.json({
      success: true,
      message: 'บันทึกการหยอดยาเรียบร้อย',
      data: {
        reminder_id: reminderId,
        medication_name: medicationExists[0].medication_name,
...
// API: ข้ามการหยอดยา
app.post('/api/medication/skip', authDoctor, async (req, res) => {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();

    const { prescription_id, patient_id, reason } = req.body;
    const currentDate = new Date().toISOString().split('T')[0];

    console.log(`💊 Skipping medication:`, { prescription_id, patient_id, reason });
    ...
    if (existingReminder.length === 0) {
      ...
      await connection.execute(
        `INSERT INTO MedicationReminders (
          reminder_id, prescription_id, patient_id, medication_name,
          scheduled_date, scheduled_time, taken_status, notes
        ) VALUES (?, ?, ?, ?, ?, TIME(NOW()), 'skipped', ?)`,
        [
          reminderId, 
          prescription_id, 
          patient_id, 
      ...
      await connection.execute(
        `UPDATE MedicationReminders 
         SET taken_status = 'skipped', notes = ?, updated_at = NOW()
         WHERE reminder_id = ?`,
        [reason || 'ข้ามการหยอดยา', reminderId]
      );
    }

    await connection.commit();

    console.log(`✅ Medication skipped: ${medicationExists[0].medication_name}`);

    res.json({
      success: true,
      message: 'บันทึกการข้ามยาเรียบร้อย',
      data: {
        reminder_id: reminderId,
        medication_name: medicationExists[0].medication_name,
        status: 'skipped',
        reason: reason || 'ข้ามการหยอดยา'
      }
    });
...
// API: ดึงสถิติการหยอดยาสำหรับ Dashboard
app.get('/api/medication/dashboard-stats', authDoctor, async (req, res) => {
  try {
    const { days } = req.query;
    const daysRange = parseInt(days) || 7;
    
    console.log(`📊 Loading medication stats for ${daysRange} days`);

    // สถิติการหยอดยาใน X วันที่แล้ว
    const [medicationStats] = await pool.execute(`
      SELECT 
        COUNT(DISTINCT pm.patient_id) as total_patients_with_medications,
        COUNT(pm.prescription_id) as total_active_prescriptions,
        COUNT(mr.reminder_id) as total_reminders,
        SUM(CASE WHEN mr.taken_status = 'taken' THEN 1 ELSE 0 END) as taken_count,
        SUM(CASE WHEN mr.taken_status = 'pending' OR mr.taken_status IS NULL THEN 1 ELSE 0 END) as pending_count,
        SUM(CASE WHEN mr.taken_status = 'overdue' THEN 1 ELSE 0 END) as overdue_count,
        SUM(CASE WHEN mr.taken_status = 'skipped' THEN 1 ELSE 0 END) as skipped_count
      FROM PatientMedications pm
      LEFT JOIN MedicationReminders mr ON pm.prescription_id = mr.prescription_id
        AND mr.scheduled_date >= DATE_SUB(CURDATE(), INTERVAL ? DAY)
      WHERE pm.status = 'active'
        AND pm.start_date <= CURDATE()
        AND (pm.end_date IS NULL OR pm.end_date >= CURDATE())
    `, [daysRange]);

    // ผู้ป่วยที่ต้องติดตาม (ไม่ได้หยอดยามากกว่า 2 วัน)
    const [needFollowUp] = await pool.execute(`
      SELECT COUNT(DISTINCT pm.patient_id) as count
      FROM PatientMedications pm
      LEFT JOIN MedicationReminders mr ON pm.prescription_id = mr.prescription_id
        AND mr.scheduled_date >= DATE_SUB(CURDATE(), INTERVAL 2 DAY)
        AND mr.taken_status = 'taken'
      WHERE pm.status = 'active'
        AND pm.start_date <= CURDATE()
        AND (pm.end_date IS NULL OR pm.end_date >= CURDATE())
        AND mr.reminder_id IS NULL
    `);

    // วันนี้ - ยาที่ต้องหยอด
    ...
    // คำนวณเปอร์เซ็นต์การปฏิบัติตาม
    const adherenceRate = stats.total_reminders > 0 ? 
      ((stats.taken_count / stats.total_reminders) * 100).toFixed(1) : 0;

    const result = {
      total_patients_with_medications: stats.total_patients_with_medications || 0,
      total_active_prescriptions: stats.total_active_prescriptions || 0,
      today_medications: todayMedications[0].total_today || 0,
      taken_count: stats.taken_count || 0,
      pending_count: stats.pending_count || 0,
      overdue_count: stats.overdue_count || 0,
      skipped_count: stats.skipped_count || 0,
      adherence_rate: adherenceRate,
      need_follow_up: needFollowUp[0].count || 0,
      days_range: daysRange
    };

    console.log('📊 Medication stats:', result);
...
// CRON Job: อัปเดตสถานะยาเป็น overdue อัตโนมัติ (รันทุก 30 นาที)
cron.schedule('*/30 * * * *', async () => {
  console.log('🔄 Running medication overdue check...');
  try {
    const connection = await pool.getConnection();
    
    // อัปเดตยาที่เลยเวลามากกว่า 2 ชั่วโมงให้เป็น overdue
    const [result] = await connection.execute(`
      UPDATE MedicationReminders 
      SET taken_status = 'overdue', updated_at = NOW()
      WHERE taken_status = 'pending' 
        AND CONCAT(scheduled_date, ' ', scheduled_time) < DATE_SUB(NOW(), INTERVAL 2 HOUR)
    `);

    if (result.affectedRows > 0) {
      console.log(`⚠️ Updated ${result.affectedRows} medications to overdue status`);
    }

    connection.release();
  } catch (error) {
    console.error('❌ Error updating overdue medications:', error);
  }
});

console.log('✅ Medication reminder APIs added successfully');
console.log('🔧 Available medication reminder endpoints:');
console.log('   GET /api/medication/alerts - ดึงการแจ้งเตือนยา');
console.log('   POST /api/medication/mark-taken - บันทึกการหยอดยาแล้ว');
console.log('   POST /api/medication/skip - ข้ามการหยอดยา');
console.log('   GET /api/medication/dashboard-stats - สถิติสำหรับ Dashboard');
console.log('   GET /api/medication/follow-up-patients - ผู้ป่วยที่ต้องติดตาม');
...
app.post('/api/patients/:patientId/iop-measurements', authDoctor, async (req, res) => {
  ...
  try {
    ...
    const {
      ...
      leftCupDiscRatio,
      rightCupDiscRatio,
      hasLeftDiscHemorrhage,
      hasRightDiscHemorrhage
    } = req.body;
    ...
    // คำสั่ง SQL ที่ถูกต้อง มี `recorded_by` เพิ่มเข้ามา
    await connection.execute(
      ...
      [
        ...
        !!hasLeftDiscHemorrhage,
        !!hasRightDiscHemorrhage,
        // เพิ่ม doctorId สำหรับคอลัมน์ recorded_by
        doctorId
      ]
    );
...
app.get('/api/appointments', authDoctor, async (req, res) => {
  try {
    ...
    console.log(`📅 Loading all appointments for doctor: ${doctorId}`);

    // แก้ไข query ให้ดึงข้อมูลผู้ป่วยมาด้วย
    const [appointments] = await pool.execute(`
      SELECT 
        a.appointment_id,
        a.appointment_date,
    ...
    if (appointments.length > 0) {
      console.log('📋 Sample appointment data:', {
        ...
        first_name: appointments[0].first_name,
        last_name: appointments[0].last_name,
        hn: appointments[0].hn
      });
    }
...
// ** API Endpoint ใหม่: สำหรับดึงรายการยาทั้งหมด **
app.get('/api/drugs', async (req, res) => {
  console.log('🔍 /api/drugs endpoint called - Fetching all medications');
  let connection; // ประกาศตัวแปร connection ข้างนอก try block
  try {
    connection = await pool.getConnection(); // ดึง connection จาก pool
    const [rows] = await connection.execute(
      `SELECT
         medication_id AS id,        -- เปลี่ยนชื่อคอลัมน์เพื่อให้ JavaScript เข้าใจง่าย
         name AS medicationName,     -- เปลี่ยนชื่อคอลัมน์เพื่อให้ JavaScript เข้าใจง่าย
         generic_name AS genericName,-- เปลี่ยนชื่อคอลัมน์เพื่อให้ JavaScript เข้าใจง่าย
         category,
         form,
         strength,
         instructions,
         storage_instructions AS storageInstructions,
         side_effects AS sideEffects
       FROM Medications
       WHERE status = 'active'      -- ดึงเฉพาะยาที่มีสถานะ active
       ORDER BY name ASC`           // เรียงตามชื่อยา
    );

    console.log(`✅ Fetched ${rows.length} medications.`);
    res.json(rows); // ส่งข้อมูลยาเป็น JSON กลับไป Frontend
  } catch (error) {
    console.error('❌ Error fetching medications:', error);
    res.status(500).json({ error: 'Failed to fetch medications', details: error.message });
  } finally {
    if (connection) { // ตรวจสอบว่า connection มีค่าก่อน release
      connection.release(); // คืน connection กลับสู่ pool
      console.log('🔚 Database connection released for /api/drugs');
    }
  }
});

// API เพื่อดึงรายการหมวดยาทั้งหมดที่ไม่ซ้ำกัน
app.get('/api/drug-categories', async (req, res) => {
    console.log('🔍 /api/drug-categories endpoint called - Fetching unique medication categories');
    let connection;
    try {
        connection = await pool.getConnection();
        // ดึงเฉพาะค่าที่ไม่ซ้ำกันจากคอลัมน์ 'category' ในตาราง 'Medications'
        // และกรองค่าที่เป็น NULL หรือ empty string ออกไป
        // ORDER BY category ASC เพื่อเรียงลำดับตามตัวอักษร
        const [rows] = await connection.execute(
            `SELECT DISTINCT category FROM Medications WHERE category IS NOT NULL AND category != '' ORDER BY category ASC`
        );
        
        // แปลงผลลัพธ์ให้อยู่ในรูปแบบ array ของ string เช่น ["Prostaglandin", "Beta Blocker"]
        const categories = rows.map(row => row.category);

        console.log(`✅ Fetched ${categories.length} unique categories.`);
        res.json(categories); // ส่งรายการหมวดหมู่เป็น JSON array กลับไป Frontend
    } catch (error) {
        console.error('❌ Error fetching drug categories:', error);
        res.status(500).json({ error: 'Failed to fetch drug categories', details: error.message });
    } finally {
        if (connection) {
            connection.release(); // คืน connection กลับสู่ pool
            console.log('🔚 Database connection released for /api/drug-categories');
        }
    }
});
